...
void Box2DDebugDraw::DrawPolygon(const b2Vec2 *vertices, int32 vertexCount, const b2Color &color) {
    for (int i = 0; i < vertexCount; i++) {
        lines.emplace_back(vertices[i].x * scale, vertices[i].y * scale, 0);
        lines.emplace_back(vertices[(i + 1) % vertexCount].x * scale, vertices[(i + 1) % vertexCount].y * scale, 0);
    }
}
...
void Box2DDebugDraw::DrawSegment(const b2Vec2 &p1, const b2Vec2 &p2, const b2Color &color) {
    lines.emplace_back(p1.x * scale, p1.y * scale, 0);
    lines.emplace_back(p2.x * scale, p2.y * scale, 0);
}
